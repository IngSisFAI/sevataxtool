...
import { DocumentCreatorCNFComponent } from "../translator-cnf/document-creator-cnf/document-creator-cnf.component";
...
export class StrategySOLVERComponent implements OnInit {
  ...
  constructor(
    private strategyCnf: DocumentCreatorCNFComponent,
  ) {}
  ...
  private recorrerVariantes(variantList: Service[], variabilityType: String) {
    for (let i = 0; i < variantList.length; i++) {
      ...
      if (variabilityType != "MandatoryVP") {
        //ESto sigfica q tiene q ser OptionalVP o AlternativeVP o VariantVP
        ...
        for (let x = 0; x < vP.length; x++) {
          ...
          for (let y = 0; y < vP.length; y++) {
            if (y != x) {
              ...
              this.gs_ds_alternative_variant_inclusion(
                vPserv_inicio,
                vPserv_destino
              );
            }
  ...
  public gs_ds_alternative_variant_inclusion(
    serv_inicio: Service,
    serv_fin: Service
  ) {
      ...
      if (dep_Type.toString() == "Require") {
        ...
        if (nombrefin == nombreAux) {
          ...
          console.log(
            "el servicio ",
            nombreinicio,
            " tiene una AlternativeVariantInclusion con ",
            nombrefin
          );
      ...
      if (dep_Type.toString() == "Use") {
        ...
        if (nombrefin == nombreAux) {
          ...
          this.outResults =
            ...
            "el servicio " +
            nombreinicio +
            " tiene una AlternativInclusion con " +
            nombrefin +
            "!!! ";
        }
      }
      if (dep_Type.toString() == "Exclude") {
        ...
        if (nombrefin == nombreAux) {
          console.log(
            "el servicio ",
            nombreinicio,
            " tiene una AlternativeVariantExclusion con ",
            nombrefin
          );
          this.outResults =
            this.outResults +
            ...
            "el servicio " +
            nombreinicio +
            " tiene una AlternativeExclusion con " +
            nombrefin +
            "!!! ";
        }
      }
  ...
  public analyse_mandatory_exclusion(datasheet: Datasheet) {
    ...
    let StrcUnder = datasheet;
    let services = StrcUnder.getServices();
    ...
    const StrcUnder2 = datasheet;
    const services2 = StrcUnder2.getServices();

    for (let i = 0; i < services2.length; i++) {
      const service = services2[i];
      this.gs_ds_mandatory_exclusion(service, mandatory_services);
    }
    return this.outResults;
  }

  public gs_ds_mandatory_exclusion(
    serv: Service,
    mandatory_services: String[]
  ) {
    const dependencies = serv.getDependencies();
    const variantPoints = serv.getVariationPoints();
    const nombre = serv.getName();
    console.log("chequeando Mandatory", nombre);

    for (let i = 0; i < dependencies.length; i++) {
      const dep_Type = dependencies[i].getDependencyType();
      const servChild = dependencies[i].getService();
      const nombreChild = servChild.getName();
      if (dep_Type.toString() === "Exclude") {
        console.log(
          nombre,
          mandatory_services.lastIndexOf(nombreChild),
          nombreChild
        );
        if (mandatory_services.lastIndexOf(nombreChild) >= 0) {
          console.log(
            "el servicio ",
            nombre,
            " tiene una Mandatory Exclusion con ",
            nombreChild
          );
          this.outResults =
            this.outResults +
    ...
    for (let i = 0; i < variantPoints.length; i++) {
      const vP = variantPoints[i].getVariants();
      for (let x = 0; x < vP.length; x++) {
        const vPserv = vP[x];
  ...
  public analyse_mandatory_inclusion(datasheet: Datasheet) {
    ...
    const mandatory_services: String[] = new Array();
    const StrcUnder = datasheet;
  ...
  public gs_ds_mandatory_inclusion(
    serv: Service,
    mandatory_services: String[]
  ) {
    const dependencies = serv.getDependencies();
      ...
      if (dep_Type.toString() === "Require") {
        ...
        if (mandatory_services.lastIndexOf(nombreChild) >= 0) {
          console.log(
            "el servicio ",
            nombre,
            " tiene una Mandatory Inclusion con ",
            nombreChild
          );
          this.outResults =
            this.outResults +
  ...
  public gs_ds_ConstraintContradition(serv: Service) {
    ...
    for (let i = 0; i < dependencies.length; i++) {
      ...
      const depType = dependencies[i].getDependencyType();
      ...
      if (this.gs_ds_compare_CC(sv, nombre, depType)) {
        ...
        this.outResults =
          ...
          "el servicio " +
          nombre +
          " tiene una Constraint Contradition con " +
          nombreAux +
          "!!! ";
      }
    }
  ...
  public gs_ds_compare_CC(
    serv: Service,
    nombre_serv: string,
    first_depType: string
  ) {
    const dependencies = serv.getDependencies();
    const nombre = serv.getName();
    let result = false;
  ...
  public analyse_transitive_inconsistency(datasheet: Datasheet) {
    console.log("INIT Transitivy Inconsistency");
  ...
  public r_ds_transitive_redundancy(serv: Service) {
    ...
    if (this.view_services.lastIndexOf(nombre) < 0) {
      ...
      const require_services: String[] = new Array();
  ...
  public service_transitive_redundancy(
    serv: Service,
    require_services: String[],
    servCheck: string
  ) {
    const dependencies = serv.getDependencies();
    const nombre = serv.getName();
      ...
      if (dep_Type.toString() === "Require") {
        if (require_services.lastIndexOf(nombreChild) >= 0) {
          console.log(
            "el servicio ",
            nombre,
            " tiene una transitive redundancy con ",
            nombreChild
          );
          this.outResults =
            this.outResults +
          ...
          this.service_transitive_redundancy(
            servChild,
            require_services,
            servCheck
          );
